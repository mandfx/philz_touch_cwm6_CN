...
void wipe_data_menu() {
    static const char* headers[] = {  "选择清除项",
                                NULL
    };
...
static void choose_custom_ors_menu(const char* volume_path) {
    ...
    static const char* headers[] = {"选择.ors脚本运行", NULL};
...
static void show_custom_ors_menu() {
    ...
    static const char* headers[] = {  "搜寻.ors脚本运行",
                                "",
                                NULL
    };
...
static void regenerate_md5_sum_menu() {
    if (!confirm_selection("不推荐该操作!!", "是的,重新生成MD5校验"))
        return;
...
void misc_nandroid_menu() {
    ...
    for (;;) {
        ...
        if (fmt == NANDROID_BACKUP_FORMAT_TGZ) {
            ...
            ui_format_gui_menu(item_compress, "压缩空间", "否");
...
static void load_theme_settings() {
    ...
    if (confirm_selection("覆盖默认设置?", "是的,") && copy_a_file(theme_file, PHILZ_SETTINGS_FILE) == 0) {
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 4: {
                ...
                if (theme_file == NULL)
                    break;
                if (confirm_selection("确认删除已选主题?", "是的,"))
...
void show_philz_settings_menu()
{
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case 2: {
                if (confirm_selection("删除已存在的su文件 ?", "是的,删除重新安装")) {
            ...
            case 6: {
                if (confirm_selection("重置所有设置到默认?", "是的,重置")) {
                    char settings_copy[PATH_MAX];
                    sprintf(settings_copy, "%s/%s", get_primary_storage_path(), PHILZ_SETTINGS_FILE2);
