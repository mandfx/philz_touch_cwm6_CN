...
int show_install_update_menu() {
    ...
    for (;;) {
        if (signature_check_enabled.value)
            ui_format_gui_menu(item_toggle_signature_check, "刷机包签名校验", "(已关闭)");
        else ui_format_gui_menu(item_toggle_signature_check, "刷机包签名校验", "(已打开)");

        if (install_zip_verify_md5.value)
            ui_format_gui_menu(item_install_zip_verify_md5, "刷机包MD5值校验", "(已关闭)");
        else ui_format_gui_menu(item_install_zip_verify_md5, "刷机包MD5值校验", "(已打开)");
...
void show_choose_zip_menu(const char *mount_point) {
    ...
    sprintf(tmp, "是的,安装 %s", BaseName(file));
...
void show_nandroid_delete_menu(const char* volume_path) {
    ...
    for(;;) {
        ...
        sprintf(tmp, "是的,删除 %s", BaseName(file));
...
static void format_ext4_or_f2fs(const char* volume) {
    ...
    if (!confirm_selection(confirm, "是的,格式化设备"))
        return;
    ...
    for (;;) {
...
#ifdef USE_F2FS
            list[mountable_volumes + formatable_volumes + 1] = "切换 f2fs <-> ext4 ";
            list[mountable_volumes + formatable_volumes + 2] = NULL;
#endif
        } else {
            list[mountable_volumes + formatable_volumes] = "格式化data(包含sdcard)";
            list[mountable_volumes + formatable_volumes + 1] = "打开U盘模式";
            list[mountable_volumes + formatable_volumes + 2] = NULL;
#ifdef USE_F2FS
            list[mountable_volumes + formatable_volumes + 2] = "切换f2fs <-> ext4 ";
        ...
        if (chosen_item == (mountable_volumes + formatable_volumes)) {
            if (!is_data_media()) {
                ...
                if (!confirm_selection("格式化data(包含sdcard)", confirm))
                    continue;
...
int show_nandroid_menu() {
    ...
    int action_entries_num = (num_extra_volumes + 1) * NANDROID_ACTIONS_NUM;
                                   // +1 for primary_path
    static const char* headers[] = { "备份/还原", NULL };
...
void show_advanced_power_menu() {
    static const char* headers[] = { "重启选项", "", NULL };
...
int show_advanced_menu() {
    ...
    list[list_index++] = "清除Dalvik Cache";   // 0
    list[list_index++] = "复制错误日志";        // 1
    list[list_index++] = "按键测试";            // 2
    list[list_index++] = "显示recovery日志";            // 3
    ...
    for (;;) {
        if (is_data_media()) {
            ...
            if (use_migrated_storage())
                list[4] = "Sdcard路径: /data/media/0";
            else list[4] = "Sdcard路径: /data/media";
        }
...
void handle_failure(int ret) {
    ...
    ui_print("/tmp/recovery.log已复制到/sdcard/clockworkmod/philz_recovery.log\n");
}
